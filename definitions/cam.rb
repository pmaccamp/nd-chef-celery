define :celery_cam, :enable => true, :virtualenv => false, :startsecs => 10, :django => false, :stopwaitsecs => 600, :options => {} do
  ...
  case params[:enable]
    when true
      ...
      if params[:django]
        ...
        celery_command = managepy + " celerycam"
      else
        celery_command = "celerycam"
      end
      ...
      Chef::Log.debug("celery_cam: generated celery_command as: " + celery_command.inspect)
      ...
      chef-supervisord_program "celerycam-#{params[:name]}" do
  ...
  service "celerycam-#{params[:name]}" do
    provider Chef::Provider::Service::Simple
    supports :start => true, :stop => true, :restart => true, :status => true
    start_command "supervisorctl start celerycam-#{params[:name]}"
    stop_command "supervisorctl stop celerycam-#{params[:name]}"
    restart_command "supervisorctl restart celerycam-#{params[:name]}"
    status_command "supervisorctl status celerycam-#{params[:name]}"
    action :nothing
  end
